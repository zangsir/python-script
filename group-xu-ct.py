...
inputfilename="/Users/zangsir/python-script/xu_data_qp2/allxu-col-ctnorm.csv"
with open(inputfilename, 'rU') as inputfile:
    reader = csv.reader(inputfile)
    # skip the header row
    next(reader, None)  

    for Syllable1, Syllable2, time, f0_norm_ct, speaker, pitch_con, syl, tone, prev in reader:
        per_label[pitch_con.strip()].append([Syllable1, Syllable2, time, f0_norm_ct, speaker, syl, tone, prev])
...
with open("/Users/zangsir/Desktop/row-yixunorm-ct.csv","w") as f:
    writer=csv.writer(f)
...
for key in per_label:
    ...
    with open("/Users/zangsir/Desktop/row-yixunorm-ct.csv","a") as f:
        writer=csv.writer(f)
        writer.writerow(a)