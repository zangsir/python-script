...
inputfilename="/Users/zangsir/Desktop/xu-analysis/allxu-col-bk.csv"
with open(inputfilename, 'rU') as inputfile:
    reader = csv.reader(inputfile)
    # skip the header row
    next(reader, None)
    for Syllable1, Syllable2, time, bark, speaker, pitch_con, syl, tone, prev in reader:
        per_label[pitch_con.strip()].append([Syllable1, Syllable2, time, bark, speaker, syl, tone, prev])
...
with open("/Users/zangsir/Desktop/row-yixu-bark.csv","w") as f:
    writer=csv.writer(f)
...
for key in per_label:
    ...
    with open("/Users/zangsir/Desktop/row-yixu-bark.csv","a") as f:
        writer=csv.writer(f)
        writer.writerow(a)