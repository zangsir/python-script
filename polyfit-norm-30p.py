...
################################################# above is history trace comments
#this one is normalized pitch (subtract mean),cent based or bark or hertz based, change input and output file name and nothing else
...
#bk,ct,hz in file name
inputfilename="./xu_data_qp2/row-yixunorm-hz.csv"

#write to the output file header
with open("poly-normed-hertz-30p.csv","w") as f:
    writer=csv.writer(f)
    e=["1st","2nd","3rd","4th","speaker","syl","tone","prev","pitch_con"]
    writer.writerow(e)
...
#here I tried  dividing data[i] by 10 above, to hopefully scale the polyfit values in a more similar range by drawing closer time(10^1) and pitch(10^2) orders of magnitude, but it's worse than original 
    ...
    with open("poly-normed-hertz-30p.csv","a") as f:
        writer=csv.writer(f)
        writer.writerow(zz)
