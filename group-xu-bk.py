...
#the input to this should contain one column for normalized pitch value, which should be one of 3: 
#1.f0_norm; 2.f0_norm_ct;3.f0_norm_bk., 

#these would vary according to their input file: 
#allxu_col_X.csv, where X is: 1.hznorm;2.ctnorm;3.bknorm.
...
inputfilename="/Users/zangsir/python-script/xu_data_qp2/allxu-col-bknorm.csv"
with open(inputfilename, 'rU') as inputfile:
    reader = csv.reader(inputfile)
    # skip the header row
    next(reader, None)  

    for Syllable1, Syllable2, time, f0_norm_bk, speaker, pitch_con, syl, tone, prev in reader:
        per_label[pitch_con.strip()].append([Syllable1, Syllable2, time, f0_norm_bk, speaker, syl, tone, prev])
...
with open("/Users/zangsir/Desktop/row-yixunorm-bk.csv","w") as f:
    writer=csv.writer(f)
...
for key in per_label:
    ...
    with open("/Users/zangsir/Desktop/row-yixunorm-bk.csv","a") as f:
        writer=csv.writer(f)
        writer.writerow(a)